...
public class VanillaRecipeCache : RecipeCache
{
    public VanillaRecipeCache() {
        foreach (Recipe recipe in Main.recipe.Where(x => x?.createItem != null && x.createItem.type != ItemID.None)) Recipes.Add(recipe);
    }

    public override void ShuffleRecipes(int seed) => throw new InvalidOperationException("Cannot shuffle a vanilla recipe cache.");
    ...
    public override void SetRecipes(IEnumerable<Recipe> recipes) {
        ...
        for (int i = 0; i < list.Count; i++) Main.recipe[i] = list[i];
    }
}